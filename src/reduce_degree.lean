...
begin
  ...
  { rw h_m_eq_m'_add_m'',
    refl },
  cases (@exists_m1 n F _ S j p h m') h_m'_in_supp_q with m1 h0m1,
  ...
  { apply comp_2 m,
    ...
    have h_m_eq_m1 : m = m1,
    { rw [h_m_eq_m'_add_m'', h_m'_m1, g_S_lem_5 h_m''_in_supp_gj x'],
      ext,
      by_cases c : j = a,
      { simp [←c, m1_j], },
      simp  [pi.add_apply, coe_tsub, coe_add, pi.sub_apply, single, coe_mk, if_neg c] },
    rwa h_m_eq_m1 },
  have h_m''_i : m'' i = 0 := g_S_lem_6 h_m''_in_supp_gj c_i_eq_j,
  have h' : m' i = m1 i := 
