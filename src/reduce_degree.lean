...
begin
  ...
  { simp only [if_pos, c_j_m, pi.add_apply, coeff_sub, sub_sub],
    ...
    { ext,
      rw [pi.add_apply, add_mul, coeff_add, mul_assoc, mul_comm (X j) (g x)],
      rw [← mul_assoc, coeff_mul_X'],
...
begin
  ...
  { rw max_eq_right h,
    ...
    exact or.resolve_left (h_h.1 i) c },
  rw not_le at h,
  rw max_eq_left h.le,
end
omit h_h
...
begin
  ...
  { apply finset.nonempty_of_ne_empty,
    simpa using h_ms },
  rw q,
  apply (add_le_add_right (total_degree_sum ms (λm , (monomial (m - single j (g j).total_degree)) (coeff m f) )) (g j).total_degree).trans,
  ...
  rw x,
  exact le_trans (monomial_degree_le_total_degree (prop_ms'' m h_m_in_ms)) (@h_total_degree_f R σ _ _ _ g j p h h_h),
  rw finsupp.le_def,
  intro i,
  by_cases c : i = j,
...
begin
  ...
  exact @h_X_2 R σ _ _ _ g hg hS hm j p h h_h,
end
