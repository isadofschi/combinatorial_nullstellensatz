...
begin
  ...
  { simp only [c_i, fin_succ_equiv_apply, polynomial.coeff_C_zero, coeff_C, ring_hom.coe_comp,
               eval₂_hom_C],
    by_cases c_m : 0 = m,
    { simp only [← c_m, if_true, eq_self_iff_true, fin.finsupp.cons_zero_zero] },
...
begin
  ...
  { rw c_j,
    ...
    { rw fin_succ_equiv_X_eq_zero,
      ...
      { simp only [finsupp.single_eq_same, finsupp.coe_tsub, pi.sub_apply],
        ...
        rw [←fin.succ_pred a c_a', fin.finsupp.cons_succ, fin.finsupp.cons_succ, finsupp.single],
  ...
  { let j' := fin.pred j c_j,
    ...
    { simp only [r, fin.finsupp.cons_succ, c_mj', if_true, not_false_iff],
      ...
      ext,
      by_cases c_a : a = 0,
      { simp [c_a, fin.finsupp.cons_zero, finsupp.single, c_j] },
...
lemma support_f_i {n : ℕ} {R : Type u} [comm_semiring R] {f : mv_polynomial (fin (n + 1)) R}
...
lemma fin_succ_equiv_support {R : Type u}[comm_semiring R] {n i : ℕ}
  ...
  ↔ ∃ m, (fin.finsupp.cons i m) ∈ f.support :=
begin
  apply iff.intro,
  intro h,
  cases ne_zero_iff.1 (polynomial.mem_support_iff.1 h) with m' hm',
  exact ⟨m', support_f_i.1 (mem_support_iff.2 hm')⟩,
  intro hm,
  cases hm with m' hm',
  simpa using ne_zero_iff.2 ⟨m', mem_support_iff.1 (support_f_i.mpr hm')⟩,
end
...
begin
  have t : (fin_succ_equiv R n f).support.nonempty, -- separate lemma!
  { by_contradiction c,
  ...
  { rw degree_of_eq,
    ...
    simpa only [fin.finsupp.cons_zero] using
    ...
    ( (@fin_succ_equiv_support R _ n (m 0) f).2
      ⟨fin.finsupp.tail m, (by rwa (fin.finsupp.cons_tail (m 0) m))⟩) },
  rw ← h,
...
begin
  ...
  { rw [polynomial.nat_degree, degree_fin_suc_equiv (by simpa only [ne.def]) ],
    simp },
end
